subroutine init_fields (n1, time,it, dt0, dt1, uk, work_nlk, vort, workvis)
  ...
  integer :: ibackup, ix, iy, iz
  integer :: filedesc, mpicode
  integer, dimension(MPI_STATUS_SIZE) :: mpistatus  
  real (kind=pr) :: time1
  real (kind=pr) :: x, y, z, r, theta, a, gamma0, x00,r00, omega
  real (kind=pr) :: xr, yr, zr, Rr, rcore, dist
...
subroutine Vorticity2Velocity (uk, work, vort)
  ...
  do iy = ca(3), cb(3)  		! ky : 0..ny/2-1 ,then, -ny/2..-1     
	...
	do iz = ca(1),cb(1)		! kz : 0..nz/2-1 ,then, -nz/2..-1           
	  kz = scalez*dble(modulo(iz+nz/2,nz)-nz/2)
	  uk(iz,ix,iy,1) = dcmplx(0d0,1d0)*( ky*work(iz,ix,iy,3) - kz*work(iz,ix,iy,2) )
	  uk(iz,ix,iy,2) = dcmplx(0d0,1d0)*( kz*work(iz,ix,iy,1) - kx*work(iz,ix,iy,3) )
	  uk(iz,ix,iy,3) = dcmplx(0d0,1d0)*( kx*work(iz,ix,iy,2) - ky*work(iz,ix,iy,1) )
	enddo
    enddo
  enddo
