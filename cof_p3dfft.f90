...
subroutine fft_initialize
  ...
  call MPI_ALLGATHER (ra, 3, mpiinteger, ra_table, 3, mpiinteger, MPI_COMM_WORLD, mpicode)
  call MPI_ALLGATHER (rb, 3, mpiinteger, rb_table, 3, mpiinteger, MPI_COMM_WORLD, mpicode)


  ! ------ Multiple one-dimensional FFTs ------
  !-- Create Cartesian topology for one-dimensional transforms
  call MPI_CART_CREATE (MPI_COMM_WORLD,nmpidims,mpidims,(/.false.,.false./),.false.,mpicommcart,mpicode)
  ...
  call dfftw_plan_many_dft_r2c(Desc_Handle_1D_f(1), 1, L, n, f, 0, 1, L, ft, 0, 1, L/2+1, FFTW_ESTIMATE)
  call dfftw_plan_many_dft_c2r(Desc_Handle_1D_b(1), 1, L, n, ft, 0, 1, L/2+1, f, 0, 1, L, FFTW_ESTIMATE)
  deallocate( f, ft )
  ...
  n = kst(2)*kst(3)
  allocate( f(0:L-1, 0:n-1), ft(0:L+1, 0:n-1) )
  call dfftw_plan_many_dft_r2c(Desc_Handle_1D_f(2), 1, L, n, f, 0, 1, L, ft, 0, 1, L/2+1, FFTW_ESTIMATE)
  call dfftw_plan_many_dft_c2r(Desc_Handle_1D_b(2), 1, L, n, ft, 0, 1, L/2+1, f, 0, 1, L, FFTW_ESTIMATE)
  deallocate( f, ft )
  ...
  n = kst(2)*kst(3)
  allocate( f(0:L-1, 0:n-1), ft(0:L+1, 0:n-1) )
  call dfftw_plan_many_dft_r2c(Desc_Handle_1D_f(3), 1, L, n, f, 0, 1, L, ft, 0, 1, L/2+1, FFTW_ESTIMATE)
  call dfftw_plan_many_dft_c2r(Desc_Handle_1D_b(3), 1, L, n, ft, 0, 1, L/2+1, f, 0, 1, L, FFTW_ESTIMATE)
  deallocate( f, ft )
