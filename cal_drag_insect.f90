...
subroutine cal_drag_parts ( time, u, partid )
  ...
  if ((iPenalization==1).and.(compute_forces==1)) then  
    ...
    call MPI_REDUCE (forcex,Insect%PartIntegrals(partid)%Force(1),1,mpireal,MPI_SUM,0,&
                    MPI_COMM_WORLD,mpicode)  
    call MPI_REDUCE (forcey,Insect%PartIntegrals(partid)%Force(2),1,mpireal,MPI_SUM,0,&
                    MPI_COMM_WORLD,mpicode) 
    call MPI_REDUCE (forcez,Insect%PartIntegrals(partid)%Force(3),1,mpireal,MPI_SUM,0,&
                    MPI_COMM_WORLD,mpicode)    
                    
    torquex = torquex*dx*dy*dz
    torquey = torquey*dx*dy*dz
    torquez = torquez*dx*dy*dz  
    call MPI_REDUCE (torquex,Insect%PartIntegrals(partid)%Torque(1),1,mpireal,MPI_SUM,0,&
                    MPI_COMM_WORLD,mpicode)  
    call MPI_REDUCE (torquey,Insect%PartIntegrals(partid)%Torque(2),1,mpireal,MPI_SUM,0,&
                    MPI_COMM_WORLD,mpicode) 
    call MPI_REDUCE (torquez,Insect%PartIntegrals(partid)%Torque(3),1,mpireal,MPI_SUM,0,&
                    MPI_COMM_WORLD,mpicode)    
