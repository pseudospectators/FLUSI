...
subroutine IBES_solver ( time, dt, beam_solid )! note this is actuall only ONE beam!!
  ...
  C2 = 2.0d0*ds*K2 - T(0)*theta_guess(1) &
     + (eta/ds**2)*(10.d0*theta_guess(0)-12.0d0*theta_guess(1)+6.0d0*theta_guess(2)-theta_guess(3) )
  !-- theta_extended(0) is the first virtual node.
  theta_guess(-1) = C2 / ( (3.0d0*eta/ds**2)-T(0) )
  !-- solve last two boundary conditions: theta_s(ns-1) = theta_ss(ns-1) = 0
  A1 = beam_guess(ns-3,5)/12.d0 - 2.d0*beam_guess(ns-2,5)/3.d0
  A2 =-beam_guess(ns-3,5)/12.d0 + 4.d0*beam_guess(ns-2,5)/3.d0 - 5.d0*beam_guess(ns-1,5)/2.d0
  !-- from these equations we can calculate the values of theta(ns) and theta(ns+1) which are added points
  theta_guess(ns) =  3.d0 * (A1-A2)/2.d0
  theta_guess(ns+1) = 12.d0 * (2.d0*A1-A2)
  ...
  theta_ss0 = ( 2.0*theta_guess(0) - 5.0d0*theta_guess(1) +4.0d0*theta_guess(2) - 1.0d0*theta_guess(3) )/ds**2
  theta_s0  = (-1.5*theta_guess(0) + 2.0d0*theta_guess(1) -0.5d0*theta_guess(2)  )/ds
  T_s0 = -eta * theta_ss0*theta_s0 + K1
  T_guess(-1) = T(1) - 2.0d0*ds*T_s0 !T_s = 0 means T(-1) = T(1)
  ...
  do while (iterate)
    ...
    F = -1.0d0*F !newton raphson is J*dx = -F
...
subroutine F_nonlinear (time, dt, dt_old,  F, theta_old, theta_dot_old, theta, T, p,&
  ...
  F = 0.d0 !to check if an index is not set. is not the case.
  ...
  F(1) = theta(0)
  F(2) = (T(1)-T(-1))/(2.0d0*ds)  +  (eta/(2.0d0*ds**3))*(theta(-1)-&
         2.0d0*theta(0)+theta(1))*(theta(1)-theta(-1))  -  K1
  F(3) = T(0)*(theta(1)-theta(-1))/(2.0d0*ds)   -   &
         (eta/(2.0d0*ds**3))*(-3.0d0*theta(-1)+10.d0*theta(0)-12.0d0*theta(1)&
         +6.0d0*theta(2)-theta(3))  -  K2
  F(4) = ( (1.0d0/12.0d0)*theta(ns-3) - (2.0d0/3.0d0)*theta(ns-2) + (2.0d0/3.0d0)*theta(ns)&
          - (1.0d0/12.0d0)*theta(ns+1) )/ds
  F(5) = ( (-1.0d0/12.0d0)*theta(ns-3) + (4.0d0/3.0d0)*theta(ns-2) - 2.5d0*theta(ns-1) + &
           (4.0d0/3.0d0)*theta(ns) + (-1.0d0/12.0d0)*theta(ns+1) )/ds**2
  F(6) = T(ns-1)

  F(7) = ( T(-1)-2.0d0*T(0)+T(1) )/(ds**2) &
    - T(0)*((theta(1)-theta(-1))/(2.0d0*ds) )**2 &
    + p(0)*((theta(1)-theta(-1))/(2.0d0*ds)) &
    + (eta/(ds**4))*(theta(1)-theta(-1))&
    * (-2.5d0*theta(0)+9.0d0*theta(1)-12.0d0*theta(2)+7.0d0*theta(3)-1.5d0*theta(4)) &
    + eta*((theta(1)-2.0d0*theta(0)+theta(-1))/(ds**2))**2 &
    + mue*(theta_dot_new(0) + alpha_t)**2  &
    - tau_s(0)

  F(8) = C3*theta_dot_old(ns-1) + C4*theta_dot_oldold(ns-1) - theta_dot_new(ns-1) &
    + (dt/(C1*mue)) * ( -(3.d0*p(ns-1)-4.d0*p(ns-2)+p(ns-3))/(2.d0*ds) &
    - eta*(theta(ns+1)-4.d0*theta(ns)+6.d0*theta(ns-1)-4.d0*theta(ns-2)+theta(ns-3))/(ds**4) &
    + (T(ns-1) + eta*( (theta(ns)-theta(ns-2))/(2.d0*ds) )**2)*(theta(ns)-2.d0*theta(ns-1)+theta(ns-2))/(ds**2) &
    + (3.*T(ns-1)-4.d0*T(ns-2)+T(ns-3))*(theta(ns)-theta(ns-2))/(2.d0*ds**2) &
    - alpha_tt*(mue**2) &
    - sigma*theta_dot_new(ns-1) &
    + tau_beam(ns-1)*(theta(ns)-theta(ns-2))/(2.0d0*ds) &
    + C2*mue*old_rhs(ns-1) )

  do i=1, ns-2
    ! first block: eqn for theta (ns+2 eqn's) [F]
    F( 8+i ) = C3*theta_dot_old(i) + C4*theta_dot_oldold(i) - theta_dot_new(i) &
    + (dt/(C1*mue))*( -1.0d0*(p(i+1)-p(i-1))/(2.d0*ds) &
    - eta*(theta(i+2)-4.d0*theta(i+1)+6.d0*theta(i)-4.d0*theta(i-1)+theta(i-2))/(ds**4) &
    + (T(i) + eta*( (theta(i+1)-theta(i-1))/(2.d0*ds) )**2)*(theta(i+1)-2.d0*theta(i)+theta(i-1))/(ds**2) &
    + (T(i+1) - T(i-1))*(theta(i+1)-theta(i-1))/(2.0d0*ds**2) &
    - alpha_tt*(mue**2) &
    - sigma*theta_dot_new(i) &
    + tau_beam(i)*(theta(i+1)-theta(i-1))/(2.0d0*ds)&
    + C2*mue*old_rhs(i))  !mue*RHS because in RHS_beameqn term is divided by MUE!!!!

    ! 2nd block: eqn's for T (ns+2 eqn's) [G]
    F( 8+(ns-2)+i ) = (T(i-1)-2.0d0*T(i)+T(i+1))/(ds**2) - T(i)*( (theta(i+1)-theta(i-1))/(2.0d0*ds) )**2 & !alles ok
    + p(i)*(theta(i+1)-theta(i-1))/(2.0d0*ds) &
    + eta*(theta(i+1)-theta(i-1))*(theta(i+2)-2.0d0*theta(i+1)+2.0d0*theta(i-1)-theta(i-2))/(2.0d0*ds**4) &
    + eta*( (theta(i+1)-2.0d0*theta(i)+theta(i-1))/(ds**2) )**2 &
...
subroutine Jacobi(time, dt, dt_old, J, T, theta, theta_old, theta_dot_old,&
  ...
  J(2,1) = 1.0d0
  !----eqn 2 (special BC 1)
  J(1,2) = (eta/(2.0d0*ds**3))*(-2.0d0*theta(-1)+2.0d0*theta(0))    ! dF2 / dTheta(-1)
  J(2,2) =-(eta/(ds**3))*(theta(1)-theta(-1))        ! dF2 / dTheta(0)
  J(3,2) = (eta/(2.0d0*ds**3))*(-2.0d0*theta(0) +2.0d0*theta(1))    ! dF2 / dTheta(1)
  J(T0_index,2)   = -0.5d0/ds            ! dF2 / dT(-1)
  J(T0_index+2,2) =  0.5d0/ds            ! dF2 / dT(+1)
  !----eqn 3 (special BC 2)
  J(1,3) = (-0.5d0*eta/(ds**3))*(-3.0d0) - T(0)/(2.0d0*ds)      ! dF3 / dTheta(-1)
  J(2,3) = (-0.5d0*eta/(ds**3))*(10.d0)          ! dF3 / dTheta(0)
  J(3,3) = (-0.5d0*eta/(ds**3))*(-12.0d0)+ T(0)/(2.0d0*ds)      ! dF3 / dTheta(+1)
  J(4,3) = (-0.5d0*eta/(ds**3))*(6.0d0)          ! dF3 / dTheta(+2)
  J(5,3) = (-0.5d0*eta/(ds**3))*(-1.0d0)          ! dF3 / dTheta(+3)
  J(T0_index+1,3) = (theta(1)-theta(-1))/(2.0d0*ds)      ! dF3 / dT(0)


  ! the following to eqns have been changed; actually, you could multiply them
  ! by any factor, but its easier to read like this
  J(ns+3,4) = (-1.0d0/12.0d0)/ds
  J(ns+2,4) = ( 2.0d0/3.0d0) /ds
  J(ns,4)   = (-2.0d0/3.0d0) /ds
  J(ns-1,4) = ( 1.0d0/12.0d0)/ds

  J(ns+3,5) = (-1.0d0/12.0d0)/ds**2
  J(ns+2,5) = ( 4.0d0/3.0d0 )/ds**2 !
  J(ns+1,5) = (-5.0d0/2.0d0 )/ds**2 !
  J(ns  ,5) = ( 4.0d0/3.0d0 )/ds**2 !
  J(ns-1,5) = (-1.0d0/12.0d0)/ds**2 !

  J(2*ns+4,6) = 1.0d0

  !-------------7th eqn: seperated 1
  J(1,7) = T(0)*(theta(1) -theta(-1))/(2.0d0*ds**2) - p(0)/(2.0d0*ds) &
           -(eta/(ds**4))*(-2.5d0*theta(0)+9.0d0*theta(1)-12.0d0*theta(2) &
           + 7.0d0*theta(3)-1.5d0*theta(4)) &
           + 2.0d0*eta*(theta(1)-2.0d0*theta(0)+theta(-1))/(ds**4)
  J(2,7) = - 2.5d0*eta*(theta(1)-theta(-1))/(ds**4) - 4.0d0*eta*(theta(1)&
           - 2.0d0*theta(0)+theta(-1))/(ds**4) &
           + 2.0d0*mue*(theta_dot_new(0)+alpha_t)*D
  J(3,7) = -T(0)*(theta(1) -theta(-1))/(2.0d0*ds**2) + p(0)/(2.0d0*ds) &
           +(eta/(ds**4))*(-2.5d0*theta(0)+9.0d0*theta(1)-12.0d0*theta(2) &
           +7.0d0*theta(3)-1.5d0*theta(4)) &
           +9.0d0*eta*(theta(1)-theta(-1))/(ds**4) + 2.0d0*eta*(theta(1)&
           -2.0d0*theta(0)+theta(-1))/(ds**4)
  J(4,7) = -12.0d0*eta*(theta(1)-theta(-1))/(ds**4)
  J(5,7) =   7.0d0*eta*(theta(1)-theta(-1))/(ds**4)
  J(6,7) = - 1.5d0*eta*(theta(1)-theta(-1))/(ds**4)
  J(ns+4,7) = 1.0d0/(ds**2)
  J(ns+5,7) = (-2.0d0-((theta(1)-theta(-1))**2)/4.0d0)/(ds**2)
  J(ns+6,7) = 1.0d0/(ds**2)

  !--------------8th eqn
  J(2*ns+4,8) = (dt/(C1*mue*ds**2))* ((theta(ns)-2.0d0*theta(ns-1)+theta(ns-2)) &
              + (3.0d0/2.0d0)*(theta(ns)-theta(ns-2)))
  J(2*ns+3,8) = -4.0d0*(dt/(C1*mue))*(theta(ns)-theta(ns-2))/(2.0d0*ds**2)
  J(2*ns+2,8) =  1.0d0*(dt/(C1*mue))*(theta(ns)-theta(ns-2))/(2.0d0*ds**2)
  J(ns+3,8)   = -dt*eta/((C1*mue)*ds**4)

  J(ns+2,8)   = (dt/(C1*mue*ds**2))   *   ( 4.0d0*eta/(ds**2) + (T(ns-1) &
              + eta*( (theta(ns)-theta(ns-2))/(2.0d0*ds) )**2) &
              + tau_beam(ns-1)*ds/(2.0d0) &   ! note we pulled 1/ds**2 out
              + eta*(theta(ns)-2.0d0*theta(ns-1)+theta(ns-2))*(theta(ns)-theta(ns-2))/(2.0d0*ds**2) &
              + (3.0d0*T(ns-1)-4.0d0*T(ns-2)+T(ns-3))/2.0d0         )

  J(ns  ,8)   = (dt/(C1*mue*ds**2))   *   ( 4.0d0*eta/(ds**2) + (T(ns-1) &
              + eta*( (theta(ns)-theta(ns-2))/(2.0d0*ds) )**2) &
              - tau_beam(ns-1)*ds/(2.0d0) &
              - eta*(theta(ns)-2.0d0*theta(ns-1)+theta(ns-2))*(theta(ns)-theta(ns-2))/(2.0d0*ds**2) &
              - (3.0d0*T(ns-1)-4.0d0*T(ns-2)+T(ns-3))/2.0d0         )

  J(ns+1,8)   = -D + (dt/(C1*mue))*( -6.0d0*eta/(ds**4) -2.0d0*(T(ns-1) &
                + eta*( (theta(ns)-theta(ns-2))/(2.0d0*ds) )**2)/(ds**2) -D*sigma)
...
!checked irreg points 16/01/11
  ...
  do i=1, ns-2
    ...
    J( l ,k)  = -D + (dt/(C1*mue)) * (-6.0d0*eta/(ds**4) &
              -(2.0d0/ds**2)*(T(i)+eta*((theta(i+1)-theta(i-1))/(2.0*ds))**2 ) - D*sigma  )
    J( l-1,k) = (dt/(C1*mue))*( 4.0d0*eta/(ds**4) + (T(i)+eta*((theta(i+1)-theta(i-1))/(2.0d0*ds))**2 )/(ds**2)  &
              - tau_beam(i)/(2.0d0*ds) &
              - eta*(theta(i+1)-2.0d0*theta(i)+theta(i-1))*(theta(i+1)-theta(i-1))/(2.0d0*ds**4) &
              - (T(i+1)-T(i-1))/(2.0d0*ds**2)  )
    J( l+1,k) = (dt/(C1*mue))*( 4.0d0*eta/(ds**4) + (T(i)+eta*((theta(i+1)-theta(i-1))/(2.0d0*ds))**2 )/(ds**2)  &
              + tau_beam(i)/(2.0d0*ds) &
              + eta*(theta(i+1)-2.0d0*theta(i)+theta(i-1))*(theta(i+1)-theta(i-1))/(2.0d0*ds**4) &
              + (T(i+1)-T(i-1))/(2.0d0*ds**2)  )
    ...
    J( l+2,k) = -eta*dt/(C1*mue*ds**4)
    !block: dF/dT
    J( m,k)   =  (dt/(C1*mue))*(theta(i+1)-2.0d0*theta(i)+theta(i-1) )/(ds**2)
    J( m-1,k) = -(dt/(C1*mue))*(theta(i+1)-theta(i-1) )/(2.0d0*ds**2)
    J( m+1,k) =  (dt/(C1*mue))*(theta(i+1)-theta(i-1) )/(2.0d0*ds**2)
  enddo
  do i=1, ns-2
    ...
    m = i + ns + 5 !index column (T) -- starts @ T(-1) = ns+5
    !block dG/dTheta
    J( l ,k)  = -4.0d0*eta* (theta(i+1)-2.0d0*theta(i)+theta(i-1))/(ds**4) + 2.0d0*mue*(theta_dot_new(i)+alpha_t)*D

    J( l-1,k) = T(i)*(theta(i+1)-theta(i-1))/(2.0d0*ds**2) -p(i)/(2.0d0*ds) +eta*(theta(i+1)-theta(i-1))/(ds**4) &
                 - eta* (theta(i+2)-2.0d0*theta(i+1)+2.0d0*theta(i-1)-theta(i-2))/(2.0d0*ds**4) &
                 + 2.0d0*eta*(theta(i+1)-2.0d0*theta(i) +theta(i-1))/(ds**4)
    J( l+1,k) = -T(i)*(theta(i+1)-theta(i-1))/(2.0d0*ds**2) +p(i)/(2.0d0*ds) -eta*(theta(i+1)-theta(i-1))/(ds**4) &
                 + eta* (theta(i+2)-2.0d0*theta(i+1)+2.0d0*theta(i-1)-theta(i-2))/(2.0d0*ds**4) &
                 + 2.0d0*eta*(theta(i+1)-2.0d0*theta(i) +theta(i-1))/(ds**4)
    J( l-2,k) = -eta*(theta(i+1)-theta(i-1))/(2.0d0*ds**4)
    J( l+2,k) =  eta*(theta(i+1)-theta(i-1))/(2.0d0*ds**4)
    !block dG/dT
    J( m,k)   = -2.0d0/(ds**2) - ( (theta(i+1)-theta(i-1) )/(2.0d0*ds) )**2
    J( m-1,k) = 1.0d0/(ds**2)
    J( m+1,k) = 1.0d0/(ds**2)
  enddo
...
subroutine RHS_beameqn (time, theta, theta_dot, pressure_beam, T, tau_beam, beam_solid)
    ...
    C2 = 2.0d0*ds*K2 - T(0)*theta(1) + (eta/ds**2)*(10.d0*theta(0)-12.0d0*theta(1)+6.0d0*theta(2)-theta(3) )

    !-- theta_extended(0) is the first virtual node.
    theta_extended(0)    = C2 / ( (3.0d0*eta/ds**2)-T(0) )
    theta_extended(1:ns) = theta !because of this, theta_extended(1) = 0 (second point added for boundarys)

    !-- solve last two boundary conditions: theta_s(ns-1) = theta_ss(ns-1) = 0
    A1 = theta(ns-3)/12.d0 - 2.d0*theta(ns-2)/3.d0
    A2 =-theta(ns-3)/12.d0 + 4.d0*theta(ns-2)/3.d0 - 5.d0*theta(ns-1)/2.d0
    !-- from these equations we can calculate the values of theta(ns) and theta(ns+1) which are added points
    theta_extended(ns+1) =  3.d0 * (A1-A2)/2.d0
    theta_extended(ns+2) = 12.d0 * (2.d0*A1-A2)
...
subroutine Tension ( time, T, T_s, theta, theta_dot, pressure, tau_beam, beam_solid)
  ...
  diagonal_main   = -2.0d0/(ds**2) - theta_s**2
  diagonal_up     =  1.0d0/(ds**2)
  diagonal_down   =  1.0d0/(ds**2)
  ! Dirichlet Condition
  diagonal_main(ns-1)  = 1.0d0 ! note this point is overwritten
  diagonal_down(ns-2)   = 0.d0 ! for the dirichlet condition
  ! Neumann Condition
  diagonal_up(0)   = 2.0d0/(ds**2)

  ! construct RHS for T-equation
  rhs(0:ns-1) = -(pressure*theta_s) -(2.d0*eta*theta_s*theta_sss)-(eta*theta_ss**2 ) &
              -mue*(theta_dot + alpha_t)**2  - tau_beam_s
  ! Dirichlet
  rhs(ns-1)     = 0.d0
  ! Neumann
  rhs(0)     = rhs(0) + T_s0 * 2.0d0/ds
...
subroutine Jacobi_num(time, dt,dt_old, J, T, theta, theta_old, &
  ...
  do i=-1,ns+1
      theta1    = theta
      theta1(i) = theta1(i)-1.0d-6
      theta2    = theta
      theta2(i) = theta2(i)+1.0d-6
