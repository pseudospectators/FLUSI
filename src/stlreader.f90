...
contains
  ...
  subroutine read_stl_file(filename, ntri, triangles, normals)
    ...
    real(kind=pr), allocatable, dimension(:,:) :: triangles, normals

    character(len=80) :: title
    integer :: iunit, irc, k, i
    integer*2 :: padding
    real*4 :: n(3),x1(3),x2(3),x3(3)

    ! only root reads from file; other procs receive data later
    if (mpirank==0) then
      write(*,*) "stlreader file="//trim(adjustl(filename))
  ...
  subroutine broadcast_stl_file(ntri, triangles, normals)
    ...
    do i=1,3
      call MPI_BCAST( normals(1,:), ntri, MPI_DOUBLE_PRECISION, 0, MPI_COMM_WORLD, mpicode )
      call MPI_BCAST( normals(2,:), ntri, MPI_DOUBLE_PRECISION, 0, MPI_COMM_WORLD, mpicode )
      call MPI_BCAST( normals(3,:), ntri, MPI_DOUBLE_PRECISION, 0, MPI_COMM_WORLD, mpicode )

      call MPI_BCAST( triangles(1,:), 3*ntri, MPI_DOUBLE_PRECISION, 0, MPI_COMM_WORLD, mpicode )
      call MPI_BCAST( triangles(2,:), 3*ntri, MPI_DOUBLE_PRECISION, 0, MPI_COMM_WORLD, mpicode )
      call MPI_BCAST( triangles(3,:), 3*ntri, MPI_DOUBLE_PRECISION, 0, MPI_COMM_WORLD, mpicode )
    enddo
  end subroutine
  ...
  subroutine normalize_stl_file( ntri, triangles, mode, scale )
    implicit none
    ...
    if (mpirank==0) then
      write(*,'("normalizing stl-file: scale=",g15.6, "x0=",3(g12.4,1x))') scale,x0,y0,z0
    endif

    ! normalize distances
    triangles = triangles / scale
    ! shift origin to x0,y0,z0
    triangles(1,:) = triangles(1,:) + x0
    triangles(2,:) = triangles(2,:) + y0
    triangles(3,:) = triangles(3,:) + z0
  ...
  real(kind=pr) function pointTriangleDistance(tri1,tri2,tri3,point,normal)
