...
!-------------------------------------------------------------------------------
! This routine writes the velocity in files called checkpoint_uk and the respective
! time.  
! So far only used for the adjoint checkpointing algorithm.
!-------------------------------------------------------------------------------
subroutine save_uk_hdf5(uk,time)
  use vars
  use hdf5_wrapper

  implicit none
  real(kind=pr),intent(in)          :: uk(ra(1):rb(1),ra(2):rb(2),ra(3):rb(3),1:nd)
  real(kind=pr),intent(in)          :: time
  ...
  write(fileName,'(A,I6.6,A)') "checkpoint_uk_",nint(1000._pr*time),".h5"

  overwrite=.true.
  do i=1,nd
    write(dsetName,*)int(i,1)
    call write_field_hdf5(fileName,dsetName,ra,rb,uk(:,:,:,i),overwrite)
    overwrite = .false.
  enddo

  !ToDO (Sophie) : screen output: "writing checkpoint data into file..."..? (If not done by hdf5_wrapper)

end subroutine save_uk_hdf5


!-------------------------------------------------------------------------------
! This routine reads the velocity in files called checkpoint_uk and the respective
! time.  
! So far only used for the adjoint checkpointing algorithm.
!-------------------------------------------------------------------------------
subroutine read_uk_hdf5(uk,time)
  use vars
  use hdf5_wrapper

  implicit none
  real(kind=pr),intent(inout)       :: uk(ra(1):rb(1),ra(2):rb(2),ra(3):rb(3),1:nd)
  real(kind=pr),intent(in)          :: time
  ...
  do i=1,nd
    write(dsetName,*)int(i,1)
    call read_field_hdf5(fileName,dsetName,ra,rb,uk(:,:,:,i))
    overwrite = .false.
  enddo

  !ToDO (Sophie) : screen output: "writing checkpoint data into file..."..? (If not done by hdf5_wrapper)

end subroutine read_uk_hdf5
