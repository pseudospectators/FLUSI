...
subroutine internal_extension_force_derivative(FJa,x,y,z,nop,springs, ke)
!FJA_CONSTRUCTION Summary of this function goes here
!   Detailed explanation goes here

do ind=1:max(springs(:,1))

    call calculate_extension_spring_force_derivative(x(springs(ind,2)), x(springs(ind,3)),&
                                                     y(springs(ind,2)), y(springs(ind,3)),&
                                                     z(springs(ind,2)), z(springs(ind,3)), &
                                                     springs(ind,4), springs(ind,5), ke(ind), &
                                                     Fxx, Fyy, Fzz, Fxy, Fxz, Fyz)


    !First point with respect to first point
                FJa(springs(ind,2),springs(ind,2)) = FJa(springs(ind,2),springs(ind,2)) + &
                                                     Fxx;

          FJa(springs(ind,2),springs(ind,2) + nop) = FJa(springs(ind,2),springs(ind,2) + nop) + &
                                                     Fxy;

        FJa(springs(ind,2),springs(ind,2) + 2*nop) = FJa(springs(ind,2),springs(ind,2) + 2*nop) +&
                                                     Fxz;

          FJa(springs(ind,2) + nop,springs(ind,2)) = FJa(springs(ind,2) + nop,springs(ind,2)) + &
                                                     Fxy;

    FJa(springs(ind,2) + nop,springs(ind,2) + nop) = FJa(springs(ind,2) + nop,springs(ind,2) + nop) + &
                                                     Fyy;

  FJa(springs(ind,2) + nop,springs(ind,2) + 2*nop) = FJa(springs(ind,2) + nop,springs(ind,2) + 2*nop) + &
                                                     Fyz;

        FJa(springs(ind,2) + 2*nop,springs(ind,2)) = FJa(springs(ind,2) + 2*nop,springs(ind,2)) + &
                                                     Fxz;

  FJa(springs(ind,2) + 2*nop,springs(ind,2) + nop) = FJa(springs(ind,2) + 2*nop,springs(ind,2) + nop) + &
                                                     Fyz;

FJa(springs(ind,2) + 2*nop,springs(ind,2) + 2*nop) = FJa(springs(ind,2) + 2*nop,springs(ind,2) + 2*nop) + &
                                                     Fzz;

    %First point with respect to second point

                FJa(springs(ind,2),springs(ind,3)) =   FJa(springs(ind,2),springs(ind,3)) + &
                                                     - Fxx;

          FJa(springs(ind,2),springs(ind,3) + nop) =   FJa(springs(ind,2),springs(ind,3) + nop) + &
                                                     - Fxy;

        FJa(springs(ind,2),springs(ind,3) + 2*nop) =   FJa(springs(ind,2),springs(ind,3) + 2*nop) + &
                                                     - Fxz;

          FJa(springs(ind,2) + nop,springs(ind,3)) =   FJa(springs(ind,2) + nop,springs(ind,3)) + &
                                                     - Fxy;

    FJa(springs(ind,2) + nop,springs(ind,3) + nop) =   FJa(springs(ind,2) + nop,springs(ind,3) + nop) + &
                                                     - Fyy;

  FJa(springs(ind,2) + nop,springs(ind,3) + 2*nop) =   FJa(springs(ind,2) + nop,springs(ind,3) + 2*nop) + &
                                                     - Fyz;

        FJa(springs(ind,2) + 2*nop,springs(ind,3)) =   FJa(springs(ind,2) + 2*nop,springs(ind,3)) + &
                                                     - Fxz;

  FJa(springs(ind,2) + 2*nop,springs(ind,3) + nop) =   FJa(springs(ind,2) + 2*nop,springs(ind,3) + nop) + &
                                                     - Fyz;

FJa(springs(ind,2) + 2*nop,springs(ind,3) + 2*nop) =   FJa(springs(ind,2) + 2*nop,springs(ind,3) + 2*nop) + &
                                                     - Fzz;

    %Second point with respect to first point

                FJa(springs(ind,3),springs(ind,2)) =   FJa(springs(ind,3),springs(ind,2)) + &
                                                     - Fxx;

          FJa(springs(ind,3),springs(ind,2) + nop) =   FJa(springs(ind,3),springs(ind,2) + nop) + &
                                                     - Fxy;

        FJa(springs(ind,3),springs(ind,2) + 2*nop) =   FJa(springs(ind,3),springs(ind,2) + 2*nop) + &
                                                     - Fxz;

          FJa(springs(ind,3) + nop,springs(ind,2)) =   FJa(springs(ind,3) + nop,springs(ind,2)) + &
                                                     - Fxy;

    FJa(springs(ind,3) + nop,springs(ind,2) + nop) =   FJa(springs(ind,3) + nop,springs(ind,2) + nop) + &
                                                     - Fyy;

  FJa(springs(ind,3) + nop,springs(ind,2) + 2*nop) =   FJa(springs(ind,3) + nop,springs(ind,2) + 2*nop) + &
                                                     - Fyz;

        FJa(springs(ind,3) + 2*nop,springs(ind,2)) =   FJa(springs(ind,3) + 2*nop,springs(ind,2)) + &
                                                     - Fxz;

  FJa(springs(ind,3) + 2*nop,springs(ind,2) + nop) =   FJa(springs(ind,3) + 2*nop,springs(ind,2) + nop) + &
                                                     - Fyz;

FJa(springs(ind,3) + 2*nop,springs(ind,2) + 2*nop) =   FJa(springs(ind,3) + 2*nop,springs(ind,2) + 2*nop) + &
                                                     - Fzz;

    %Second point with respect to second point

                FJa(springs(ind,3),springs(ind,3)) =   FJa(springs(ind,3),springs(ind,3)) + &
                                                       Fxx;

          FJa(springs(ind,3),springs(ind,3) + nop) =   FJa(springs(ind,3),springs(ind,3) + nop) + &
                                                       Fxy;

        FJa(springs(ind,3),springs(ind,3) + 2*nop) =   FJa(springs(ind,3),springs(ind,3) + 2*nop) + &
                                                       Fxz;

          FJa(springs(ind,3) + nop,springs(ind,3)) =   FJa(springs(ind,3) + nop,springs(ind,3)) + &
                                                       Fxy;

    FJa(springs(ind,3) + nop,springs(ind,3) + nop) =   FJa(springs(ind,3) + nop,springs(ind,3) + nop) + &
                                                       Fyy;

  FJa(springs(ind,3) + nop,springs(ind,3) + 2*nop) =   FJa(springs(ind,3) + nop,springs(ind,3) + 2*nop) + &
                                                       Fyz;

        FJa(springs(ind,3) + 2*nop,springs(ind,3)) =   FJa(springs(ind,3) + 2*nop,springs(ind,3)) + &
                                                       Fxz;

  FJa(springs(ind,3) + 2*nop,springs(ind,3) + nop) =   FJa(springs(ind,3) + 2*nop,springs(ind,3) + nop) + &
                                                       Fyz;

FJa(springs(ind,3) + 2*nop,springs(ind,3) + 2*nop) =   FJa(springs(ind,3) + 2*nop,springs(ind,3) + 2*nop) + &
                                                       Fzz;
