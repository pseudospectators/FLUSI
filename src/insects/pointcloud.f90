subroutine mask_from_pointcloud(points, normals, xx0, ddx, mask, safety, d0, mask_color, color)
  implicit none
  ! point cloud data (points and normals)
  real(kind=pr),dimension(1:,1:),intent(in):: points, normals
  ! xx0 and spacing of grid
  real(kind=pr),dimension(1:3),intent(in):: xx0, ddx
  real(kind=pr),intent(inout):: mask(0:,0:,0:)
    ...
    do iy = lbounds(2), ubounds(2)
      do ix = lbounds(1), ubounds(1)
          ...
          if (tmp<1.0d7) then
              ...
              mask(ix,iy,iz) = steps( tmp-d0, 0.d0 )
          else
              mask(ix,iy,iz) = 0.0d0
          endif
      enddo
