...
subroutine rigid_solid_rhs(time,it,Insect)
  ...
  type(diptera),intent(inout)::Insect
  real (kind=pr) :: m,g, Jx, Jy,Jz,Jxy, s, Tx, Ty, Tz, T_wing_g(1:3), T_wing_w(1:3)
  real (kind=pr) :: omx,omy,omz, Jxx,Jyy,Jzz
  real (kind=pr), dimension(0:3) :: ep
  real (kind=pr), dimension(1:3) :: ROT, torque_body
  real (kind=pr), dimension(1:3,1:3) :: M_body, M_wing_l
  ...
  torque_body = matmul( M_body, GlobalIntegrals%torque + GlobalIntegrals%torque_unst)
...
subroutine rigid_solid_init(time, Insect)
  ...
  real(kind=pr), intent(in) :: time
  type(diptera), intent(inout) :: Insect
  real (kind=pr) :: yaw,pitch,roll,a,t,p
  integer :: mpicode
  real (kind=pr), dimension(0:3) :: ep
...
subroutine muscle_moment( time, insect )
  ...
  real(kind=pr), intent(in) :: time
  type(diptera), intent(inout) :: Insect
  real(kind=pr)::phi,m, t,y2,y1,x2,x1
